...
# to install napari-biomag-annotator on your Linux PC
...
VENVNAME=$PARENT_FOLDER/napariAnnotatorEnv
echo $VENVNAME
...
if [ -d "$VENVNAME" ]; then
  if [ -d "$VENVNAME/lib/site-packages/pip" ]; then
    ...
    source $VENVNAME/bin/activate && echo "Activated virtual environment successfully: $VENVNAME" && pip install napari[all] && pip install napari-biomag-annotator && echo "Installed pip packages successfully" && napari
  else
